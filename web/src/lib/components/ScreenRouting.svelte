<script lang="ts">
  ...
  const MODES = [
    ...
    { id: "redirect", title: "REDIRECT", tag: "TCP · рекоменд.", desc: "Прозрачный перехват через iptables nat. Только TCP. Лучший баланс совместимости и простоты." },
    { id: "tproxy", title: "TProxy", tag: "TCP+UDP", desc: "Прозрачный перехват TCP и UDP. Нужен для QUIC/HTTP3, но требовательнее к ядру." },
  ...
  const MODE_HINT: Record<string, string> = {
    ...
    redirect: "Только трафик к доменам и подсетям ниже идёт через VPN — остальное напрямую. Правила восстанавливаются после перезагрузки.",
    tproxy: "TCP и UDP к доменам и подсетям ниже идут через VPN. Подходит для приложений с QUIC.",
